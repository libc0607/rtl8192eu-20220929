...
{
	u32 DataAndAddr = 0, MaskforPhySet = 0;
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(Adapter);
	BB_REGISTER_DEFINITION_T *pPhyReg = &pHalData->PHYRegDef[eRFPath];
	u32 NewOffset;
...
{/* Move from _InitAdaptiveCtrl_8192E */
	/* SIFS (used in NAV) */
	u16 value16;
...
{/* Move from HW_VAR_RESP_SIFS */
	...
	PlatformEFIOWrite1Byte(Adapter, REG_SIFS_CTX_8192E+1, val8[0]);
	/* SIFS for OFDM consecutive tx like CTS data! */
	PlatformEFIOWrite1Byte(Adapter, REG_SIFS_TRX_8192E+1, val8[1]);

	PlatformEFIOWrite1Byte(Adapter, REG_SPEC_SIFS_8192E+1, val8[0]);
	PlatformEFIOWrite1Byte(Adapter, REG_MAC_SPEC_SIFS_8192E+1, val8[0]);

	/* Revise SIFS setting due to Hardware register definition change. */
	PlatformEFIOWrite1Byte(Adapter, REG_RESP_SIFS_OFDM_8192E+1, val8[0]);
	PlatformEFIOWrite1Byte(Adapter, REG_RESP_SIFS_OFDM_8192E, val8[0]);
}
#endif
