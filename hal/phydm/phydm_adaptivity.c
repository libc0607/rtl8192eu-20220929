...
void phydm_set_edcca_threshold(void *dm_void, s8 H2L, s8 L2H)
{
	...
	if (dm->support_ic_type & ODM_IC_JGR3_SERIES) {
		...
		//RTW_INFO("phydm_set_edcca_threshold(), H2L=%d, L2H=%d, override_en=%d, l2h_override=%d\n", H2L, L2H, edcca_override_en, edcca_override_l2h_dbm);
		if (edcca_override_en) {
			odm_set_bb_reg(dm, R_0xc4c, MASKBYTE0, (u8)(edcca_override_l2h_dbm+110));
			odm_set_bb_reg(dm, R_0xc4c, MASKBYTE2, (u8)(edcca_override_l2h_dbm+110-8));
		} else {
